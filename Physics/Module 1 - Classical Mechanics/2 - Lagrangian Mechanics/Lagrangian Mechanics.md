...
> [!def] Lagrangian
> In mechanics, the [[3.8 - Calculus of Variation#^8dfdfb]] is the [[1.5 - Work and Energy#^95e31f]] of the system minus the [[1.5 - Work and Energy#^0663fa]] of the system.
>
...
> The [[3.5 - Derivatives#^7ced7c]] of the [[#^86cac2]] $\mathcal{L}(q,\dot{q}_{i},t)$ with respect to $\dot{q}_{i}$ is called the **generalized [[1.6 - Linear Momentum and Collisions#^57c92e]].**
> $$\frac{\partial \mathcal{L}}{\partial \dot{q}_{i}}=p_{i}$$

^0b0692

> [!def] Generalized Force
> The [[3.5 - Derivatives#^7ced7c]] of the [[#^86cac2]] $\mathcal{L}(q,\dot{q}_{i},t)$ with respect to coordinate $q_{i}$ is called the **generalized [[1.3 - Newton's Laws#^73f9cb]].**
> $$\frac{\partial \mathcal{L}}{\partial q_{i}}=F_{i}$$

^0b0692

> [!thm] Hamilton's Principle
> The motion of a particle with position $\mathbf{q}=\left<q_{1},\dots q_{k}\right>$ in any generalized coordinates will always minimize the action integral of the [[#^86cac2]].
>
> For each $i$th coordinate, there will be an [[3.8 - Calculus of Variation#^5a5350]] using the [[#^0b0692]] $\mathcal{L}_{q_{i}}=F_{i}$ and [[#^0b0692]] $\mathcal{L}_{\dot{q}_{i}}=p_{i}$.
> $$F_{i}=\frac{d}{dt}\,p_{i}$$
...
Consider a particle freely moving with $\text{KE}=\frac{1}{2}m\,\dot{x}^{2}, \ \text{PE}=U(x)$. Then, we can write a Lagrangian $\mathcal{L}=\frac{1}{2}m\,\dot{x}^{2}-U(x)$. Then, and $F_{x}=\mathcal{L}_{q}=-\frac{dU}{dx}$ and $p=\mathcal{L}_{\dot{q}}=m\,\dot{x}$. Then, by [[#^8b76e4]], we can say $F=\dot{p}$, or

$$
-\frac{dU}{dx}=\,m\ddot{x}
$$

Which is identical to the equations for [[1.3 - Newton's Laws#^aee106]] and [[1.5 - Work and Energy#^0663fa]].
...
> In any classical system with independent variables $\{q_{1},\dots,q_{k} \}$, if a [[#^86cac2]] is **explicitly invariant** over any variable, it corresponds to a conserved quantity.

^2afb30

> [!cor] Conservation of Energy
> By [[#^2afb30]], if $\mathcal{L}(q,\dot{q})$ is explicitly invariant over time $t$, it results in [[1.5 - Work and Energy#^bcffb2]].
>
...
> By [[#^2afb30]], if $\mathcal{L}(\dot{q}),t$ is explicitly invariant over $q$, it results in [[1.6 - Linear Momentum and Collisions#^bf7448]]

> [!cor] Conservation of Angular Momentum
> By [[#^2afb30]], if $\mathcal{L}(q,t)$ is explicitly invariant over $\dot{q}$, it results in [[1.8 - Angular Momentum#^833af3]].
